(function ($) {
  ...
  $(document).on("click", "#publish", function (e) {
    if ($("#title").val() === "") {
      e.preventDefault();
      alert(
        "Please ensure you give the map post a title. It will not save properly otherwise."
      );
    }
  });
  ...
  $(document).on("change", "#wp_mapbox_gl_js_dataset_list", function (e) {
    ...
    if (e.target.value !== "") {
      ...
      $.ajax({
        url:
          "https://api.mapbox.com/datasets/v1/" +
          $("#wp_mapbox_gl_js_username").val() +
          "/" +
          e.target.value +
          "/features?access_token=" +
          $("#wp_mapbox_gl_js_secret_token").val(),
      }).done(function (data) {
        ...
        data.features.forEach(function (feature, index) {
          ...
          $("#wp_mapbox_gl_js_feature_list").append(
            '<option value="' + feature.id + '">' + featureName + "</option>"
          );
        });
      });
    } else {
  ...
  $(document).on(
    "click",
    ".wp-mapbox-gl-js-advanced-options-button",
    function (e) {
      e.preventDefault();
  ...
  if ($("#toggle_wp_mapbox_gl_js_credentials").length) {
    $(document).on(
      "click",
      "#toggle_wp_mapbox_gl_js_credentials",
      function (e) {
        e.preventDefault();
  ...
  $(document).on("click", "#wp-mapbox-gl-js-export-button", function (e) {
    ...
    if ($("#wp-mapbox-gl-js-category-export").val() !== "") {
      $.ajax({
        url:
          $("#wp-mapbox-gl-js-plugins").val() +
          "/wp-mapbox-gl-js/admin/partials/wp-mapbox-gl-js-print.php?gl_js_maps_category=" +
          $("#wp-mapbox-gl-js-category-export").val(),
        method: "GET",
        dataType: "json",
        ...
        downloadTextFile(
          JSON.stringify(data),
          $("#wp-mapbox-gl-js-category-export").val() + ".json"
        );
      });
    }
  });
  ...
  if (window.location.href.indexOf("wp-mapbox-gl-js-settings") > -1) {
    try {
      ...
      $.ajax({
        url:
          "https://api.mapbox.com/styles/v1/mapbox/light-v9?access_token=" +
          access_token,
      })
        .done(function (data) {
          $(".dashicons-yes").show();
      ...
      mapObject.mapData.forEach(function (feature) {
        if (feature.id) {
          if (
            ...
            feature.properties.marker_icon_anchor === "center"
          ) {
          ...
          if (
            feature.properties.marker_icon_url.indexOf("black_default.png") > -1
          ) {
          ...
          $.ajax({
            ...
            url:
              ...
              $("#wp_mapbox_gl_js_username").val() +
              "/" +
              $("#wp_mapbox_gl_js_dataset_list").val() +
              "/features/" +
              feature.id +
              "?access_token=" +
              $("#wp_mapbox_gl_js_secret_token").val(),
  ...
  if (
    $("#wp_mapbox_gl_js_access_token").length &&
    $("#wp-mapbox-gl-js-dataset-map").length
  ) {
    var access_token = $("#wp_mapbox_gl_js_access_token").val();
    mapboxgl.accessToken = access_token;
    var map = new mapboxgl.Map({
    ...
    $(document).on("click", "#wp-mapbox-gl-js-to-dataset-delete", function (e) {
      ...
      if (
        confirm(
          "Are you sure you want to completely delete these features in Mapbox? This cannot be reversed. \n\n NOTE: it will take up to 10 minutes for Mapbox to show the changes in its dataset."
        )
      ) {
        var updatedFeatures = [];
        var mapObject = JSON.parse($("#wp_mapbox_gl_js_map_object").val());
    ...
    $(document).on("click", "#wp-mapbox-gl-js-to-dataset-update", function (e) {
      ...
      if (
        confirm(
          "Are you sure you want to send the features from this WP GL JS post to Mapbox? This is not easily reversed. \n\n NOTE: it will take up to 10 minutes for Mapbox to show the changes in its dataset."
        )
      ) {
        var updatedFeatures = [];
        var mapObject = JSON.parse($("#wp_mapbox_gl_js_map_object").val());
        mapObject.mapData.forEach(function (feature) {
          if (feature.id) {
            ...
            if (
              feature.properties.marker_icon_url.indexOf("black_default.png") >
              -1
            ) {
              delete feature.properties.marker_icon_url;
            }
    ...
    $(document).on(
      "click",
      "#wp-mapbox-gl-js-from-dataset-update",
      function (e) {
        e.preventDefault();
        if (
          confirm(
            "Are you sure you want to overwrite the features in this WP GL JS post using the latest data in your Mapbox dataset?"
          )
        ) {
          var mapObject = JSON.parse($("#wp_mapbox_gl_js_map_object").val());
          var currentNumber = parseInt(
            $("#wp-mapbox-gl-js-from-dataset-update-updated-number").text()
          );
          var numberToMatch = 0;
          mapObject.mapData.forEach(function (feature) {
            if (feature.id) {
              ...
              $.ajax({
                ...
                mapObject.mapData.forEach(function (thisMapData) {
                  if (thisMapData.id === data.id) {
                    ...
                    for (var property in data.properties) {
                      thisMapData.properties[property] =
                        data.properties[property];
                    }
                    currentNumber += 1;
                    $("#wp-mapbox-gl-js-from-dataset-update-updated").fadeIn();
                    $(
                      "#wp-mapbox-gl-js-from-dataset-update-updated-number"
                    ).fadeIn();
                    $(
                      "#wp-mapbox-gl-js-from-dataset-update-updated-number"
                    ).text(currentNumber);
                ...
                if (currentNumber === numberToMatch) {
                  setTimeout(function () {
                    $("#wp_mapbox_gl_js_map_object").val(
