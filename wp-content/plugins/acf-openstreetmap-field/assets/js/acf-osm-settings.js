(function ($, arg, exports) {
  ...
  L.Control.ResetLayers = L.Control.extend({
    ...
    onRemove: function () {
      L.DomEvent.off(this._link, "click", L.DomEvent.stopPropagation)
  ...
  $(document).on("acf-osm-map-init", function (e) {
    ...
    function addLayer(provider_key) {
      ...
      layerConfig = options.layer_config[provider_key.split(".")[0]] || {
        options: {},
      };
    ...
    $(document)
      ...
      .on("change", '.osm-disable[type="checkbox"]', function (e) {
        $(this)
          .closest(".acf-osm-setting")
          .toggleClass("disabled", $(this).prop("checked"));
      });
  });
